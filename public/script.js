...
async backgroundPreload() {
    ...
    showAuth() {
        // Make sure both screens exist before trying to modify them
        ...
        if (authScreen && appScreen) {
            ...
            if (registerPin) registerPin.value = '';
            
            // Reset any error messages
            const errorMessages = document.querySelectorAll('.error-message');
            errorMessages.forEach(msg => msg.textContent = '');
        } else {
            console.error('Auth or App screen elements not found!');
        }
    }
    ...
    switchAuthTab(tab) {
        ...
        // Make sure elements exist before operating on them
        if (authTabs && activeTab && loginForm && registerForm) {
            // Switch active tab
            ...
            console.error('Required auth elements not found!');
        }
    }
    ...
    try {
      ...
      targetGrid.innerHTML = images.map(img => `
        <div class="image-card" data-id="${img._id}" onclick="app.openImageModal('${img._id}', '${img.cloudUrl}', '${img.isPinned}', '${img.isLocked}')">
          ...
          <button class="image-delete-btn" onclick="event.stopPropagation(); app.deleteImage('${img._id}')" title="Delete">
            <i class="material-icons">delete</i>
          </button>
        </div>
      `).join('');
  ...
  async deleteImage(id) {
    ...
    try {
      ...
      // Reload both regular and locked
  ...
  async toggleImagePin(id) {
    try {
      await fetch('/api/images/' + id + '/pin', {
        method: 'POST',
        credentials: 'include'
      });
      await this.loadImages('regular');
  ...
  async toggleImageLock(id) {
    try {
      await fetch('/api/images/' + id + '/lock', {
        method: 'POST',
        credentials: 'include'
      });
      await this.loadImages('regular');
  ...
  openImageModal(id, url, isPinned, isLocked) {
    const modal = document.getElementById('image-modal');
    const img = document.getElementById('image-modal-img');
    img.src = url;
    modal.dataset.imageId = id;
    modal.dataset.isPinned = isPinned;
    modal.dataset.isLocked = isLocked;
    modal.classList.remove('hidden');

    // Update button states
    const pinBtn = document.getElementById('image-pin-btn');
    const lockBtn = document.getElementById('image-lock-btn');
    pinBtn.style.color = isPinned === 'true' ? '#FF9500' : '#89999A';
    lockBtn.textContent = isLocked === 'true' ? 'lock_open' : 'lock';
  }
  ...
  // Image lock password gate
  async checkImageLockSetup() {
    try {
